...
<head>
	...
	<!-- start: Meta -->
	<meta charset="utf-8">
	<title>Galería Categoría Privados</title>
	...
	<!-- start: Header -->
		...
		<div class="row-fluid">
			...
			<ul class="breadcrumb">
				...
				<li><a href="#">Galería Categoría Privados</a></li>
			</ul>
                        <?php
                            include "config.php";
                            error_reporting(E_ALL);
                            $res = $mysqli->query("SELECT id_category,name_category FROM categorys where id_category='6'");
                            $mysqli->close();
					...
					<div class="box-content">
						<div class="masonry-gallery">
                                                    <?php
                                                        include "config.php";
                                                        error_reporting(E_ALL);
                                                        $res2 = $mysqli2->query("SELECT DISTINCT c.id_content,c.tittle,c.route,c.description FROM content_dron_category AS cpr LEFT JOIN content AS c ON c.id_content = cpr.id_content  WHERE cpr.id_category ='" . $id_pr . "'");
                                                         $mysqli2->close();
